...
//                       &iotc_mqttlogic_subscribe_callback, NULL);
    ...
    case IOTC_CONNECTION_STATE_OPEN_FAILED:
        ESP_LOGI(TAG, "ERROR!\tConnection has failed reason %d\n\n", state);
    ...
    case IOTC_CONNECTION_STATE_CLOSED:
        ...
        if (state == IOTC_STATE_OK) {
            ...
            ESP_LOGI(TAG, "connection closed - reason %d!\n", state);
...
static void mqtt_task(void *pvParameters)
{
    ...
    * GCP IoT Core Service. */
    ...
    iotc_state_t state = iotc_create_iotcore_jwt(
                             ...
                             3600, &iotc_connect_private_key_data, jwt,
